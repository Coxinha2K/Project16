def init(args):
    ...
    found = False
    userlist = json.loads(authority.catfile("P0", "/system/config/userlist")[1])
    while not found:
    ...
    if not authority.objectexists(userdata[1], userdata[0]):
        if debug:
    ...
    elif not authority.objectexists(appdata[1], appdata[0]):
        if debug:
    ...
    authority.mkfile("P2", "/userlist", json.dumps(userlist))
    if debug:
    ...
    while running:
        ...
        if cmd.lower() in ("exit", "quit"):
            ...
            project16_version = authority.systemini()["OSVersion"]
            core16_version = authority.bootcfg()["Core16"]["Version"]
            print(f"Running Project16 {project16_version}")
            print(f"On top of Core16 {core16_version.replace('-', ' ')}")
            ...
            if authority.objectexists(appdata[1], folder):  
                if debug:
            ...
            code = authority.catfile(f"{folder}/main.pm", appdata[1])[1]
            if debug:
                print(f"[Project16] App code found, calling Platinum")
                
            platinum = types.ModuleType("platinum")
            pcode = authority.catfile("P1", "/bin/platinum")[1]
